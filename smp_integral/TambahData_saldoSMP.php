<?php
    ...
    $lokasi3 = "Tambah Data SMP Integral";
    ...
    $kodetambahpengeluaranSMP = kodetambahpengeluaranSMP();
    $kodetambahYayasan = kodetambahYayasan();

    $hasil1=mysqli_query($koneksi,"SELECT anggaran FROM db_validasi  where id_usaha = 'DTU004'");
    while ($jumlah1=mysqli_fetch_array($hasil1)){
    $arrayhasil1[] = $jumlah1['anggaran'];
    }
    $jumlahhasil1 = array_sum($arrayhasil1);

    $hasil2=mysqli_query($koneksi,"SELECT * FROM db_validasi where id_usaha = 'DTU004'");
    while ($jumlah2=mysqli_fetch_array($hasil2)){
    $arrayhasil2[] = $jumlah2['sub_total'];
    }
    $jumlahhasil2 = array_sum($arrayhasil2);
    
    $hasil3=mysqli_query($koneksi,"SELECT SUM(anggaran) AS totalkeseluruhan FROM db_validasi where id_usaha = 'DTU004'");
    ...
    $hasil4=mysqli_query($koneksi,"SELECT anggaran * 0.80 AS totalkeseluruhan FROM db_validasi where id_usaha = 'DTU004'");
...
<div class="container-fluid">
    <h2 align="center" class="pt-3 pb-3">Input Pengeluaran SMP Integral</h2>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="form-group ">
                        ...
                        <input type="text" class="form-control" name="id_data_output" value="<?=$kodetambahpengeluaranSMP?>" readonly>
                    ...
                    <div class="form-group ">
                        ...
                        <input type="text" class="form-control" name="" value="SMP INTEGRAL" readonly>
                    ...
                    <label for="">Kategori  </label>
                        <select name="id_kategori" class="form-control select-dropdown" required>
                            ...
                            $query = mysqli_query($koneksi, "SELECT * FROM db_kategori_pengeluaran WHERE id_usaha = 'DTU004' ORDER BY id_kategori DESC LIMIT 2");
...
<?php
    ...
    if (isset($_POST['submit'])) {
        if ($_POST['kd_jumlah'] <= $_POST['max']) {
            if (tambahdatapengeluaransaldoSMP($_POST) > 0){
                echo "
                <script>
                alert('Pengeluaran Sudah Berhasil Di Tambah');
                document.location.href = 'EditData_pengeluaranSMP.php';         
                </script>
                ";
            }   else {
                echo "
                ...
                document.location.href = 'TambahData_pengeluaranSMP.php';         
                </script>
                ";
                echo("<br>");
