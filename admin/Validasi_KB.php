<?php
    ...
    $namasumber1 = mysqli_query($koneksi,"SELECT * FROM `db_usaha` where id_usaha= 'DTU002' ");
    $sumbern1= mysqli_fetch_assoc($namasumber1);

    $jumlah1 = mysqli_query($koneksi,"SELECT SUM(kd_jumlah)  from db_data_input 
    WHERE id_usaha = 'DTU002'AND id_pp = 1
    ");
    ...
    $hasil2=mysqli_query($koneksi,"SELECT * FROM db_data_output where id_usaha = 'DTU002'");
    ...
    $db_user= query("SELECT id_validasi, id_usaha, sub_total, anggaran FROM db_validasi WHERE id_validasi = 'VAK001'");
    ...
    $hasil4=mysqli_query($koneksi,"SELECT * FROM db_data_input  where id_usaha = 'DTU002'");
    while ($jumlah4=mysqli_fetch_array($hasil4)){
    $arrayhasil4[] = $jumlah4['kd_jumlah'];
    }
    $jumlahhasil4 = array_sum($arrayhasil4);

    $hasil5=mysqli_query($koneksi,"SELECT * FROM db_data_output  where id_usaha = 'DTU002'");
        ...
        <div class="col-sm-8 col-lg-8 ">
            ...
            <table class="table table-striped table-hover table-bordered table-align-middle" id="data">
                ...
                <tr align="center">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="Deletedata_validasiKB.php?id=<?=$data['id_validasi']?>" onclick="return confirm('Apakah anda ingin menghapus data ini ?')" class="btn btn-danger"><i class="nav-icon fa fa-trash"></i> Hapus</a>
                        </div>
                    </td>
                </tr>
                    <?php } ?>
        ...
        <div class="col-sm-8 col-lg-8 ">
            ...
            <table class="table table-striped table-hover table-bordered table-align-middle" id="data">
                ...
                <tr align="center">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="Deletedata_saldoKB.php?id=<?=$data['id_validasi']?>" onclick="return confirm('Apakah anda ingin menghapus data ini ?')" class="btn btn-danger"><i class="nav-icon fa fa-trash"></i> Hapus</a>
                        </div>
                    </td>
                </tr>
                    <?php } ?>
...
<?php
    ...
    if (isset($_POST['submit'])) {
        if (tambahValidasiKB($_POST) > 0){
            echo "
            <script>
            alert('Validasi Data Berhasil');
            document.location.href = 'Validasi_KB.php';
            </script>
            ";
        } else {
            echo "
