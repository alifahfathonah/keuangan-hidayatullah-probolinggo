<?php
    ...
    $hasil2=mysqli_query($koneksi,"SELECT * FROM db_data_output where id_usaha = 'DTU006'");
    ...
    $db_user= query("SELECT id_validasi, id_usaha, sub_total, anggaran FROM db_validasi WHERE id_validasi = 'VAZ001'");
    ...
    $hasil4=mysqli_query($koneksi,"SELECT * FROM db_data_input  where id_usaha = 'DTU006'");
    while ($jumlah4=mysqli_fetch_array($hasil4)){
    $arrayhasil4[] = $jumlah4['kd_jumlah'];
    }
    $jumlahhasil4 = array_sum($arrayhasil4);

    $hasil5=mysqli_query($koneksi,"SELECT * FROM db_data_output  where id_usaha = 'DTU006'");
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="form-group ">
                        <input type="text" class="form-control" name="id_validasi" value="<?=$kodevalidasiMZ?>" hidden>
        ...
        <div class="col-sm-8 col-lg-8 ">
            ...
            <table class="table table-striped table-hover table-bordered table-align-middle" id="data">
                ...
                <tr align="center">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="Deletedata_validasiMZ.php?id=<?=$data['id_validasi']?>" onclick="return confirm('Apakah anda ingin menghapus data ini ?')" class="btn btn-danger"><i class="nav-icon fa fa-trash"></i> Hapus</a>
                        </div>
                    </td>
                </tr>
                    <?php } ?>
        ...
        <!-- <div class="col-sm-8 col-lg-8 ">
            ...
            <table class="table table-striped table-hover table-bordered table-align-middle" id="data">
                ...
                <tr align="center">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="Deletedata_saldoMZ.php?id=<?=$data['id_validasi']?>" onclick="return confirm('Apakah anda ingin menghapus data ini ?')" class="btn btn-danger"><i class="nav-icon fa fa-trash"></i> Hapus</a>
                        </div>
                    </td>
                </tr>
                    <?php } ?>
...
<?php
    ...
    if (isset($_POST['submit'])) {
        if (tambahValidasiMZ($_POST) > 0){
            echo "
            <script>
            alert('Validasi Data Berhasil');
            document.location.href = 'Validasi_MZ.php';
            </script>
            ";
        } else {
            echo "
