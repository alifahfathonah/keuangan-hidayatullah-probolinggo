<?php
    ...
    $lokasi3 = "Validasi Pemasukan Pesantren Tahfidz DH";
    ...
    $kodevalidasiPesantren = kodevalidasiPesantren();
    $kodevalidasiYayasan = kodevalidasiYayasan();


    $namasumber1 = mysqli_query($koneksi,"SELECT * FROM `db_usaha` where id_usaha= 'DTU005' ");
    $sumbern1= mysqli_fetch_assoc($namasumber1);

    $jumlah1 = mysqli_query($koneksi,"SELECT SUM(kd_jumlah)  from db_data_input 
    WHERE id_usaha = 'DTU005'AND id_pp = 1
    ");
    // menghitung data barang
    $jumlah_barang = mysqli_num_rows($jumlah1);

    $hasil1=mysqli_query($koneksi,"SELECT * FROM db_data_input where id_usaha = 'DTU005'");
    while ($jumlah1=mysqli_fetch_array($hasil1)){
    $arrayhasil1[] = $jumlah1['kd_jumlah'];
    }
    $jumlahhasil1 = array_sum($arrayhasil1);

    $hasil2=mysqli_query($koneksi,"SELECT * FROM db_data_output where id_usaha = 'DTU005'");
    ...
    $db_user= query("SELECT id_validasi, id_usaha, sub_total, anggaran FROM db_validasi WHERE id_validasi = 'VAP001'");
    ...
    $hasil4=mysqli_query($koneksi,"SELECT * FROM db_data_input  where id_usaha = 'DTU005'");
    while ($jumlah4=mysqli_fetch_array($hasil4)){
    $arrayhasil4[] = $jumlah4['kd_jumlah'];
    }
    $jumlahhasil4 = array_sum($arrayhasil4);

    $hasil5=mysqli_query($koneksi,"SELECT * FROM db_data_output  where id_usaha = 'DTU005'");
...
<div class="container-fluid">
    <br>
    <h2 align="center">Validasi Pemasukan Pesantren Tahfidz DH </h2>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="form-group ">
                        <input type="text" class="form-control" name="id_validasi" value="<?=$kodevalidasiPesantren?>" hidden>
        ...
        <div class="col-sm-8 col-lg-8 ">
            ...
            <table class="table table-striped table-hover table-bordered table-align-middle" id="data">
                ...
                <tr align="center">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="Deletedata_validasiPesantren.php?id=<?=$data['id_validasi']?>" onclick="return confirm('Apakah anda ingin menghapus data ini ?')" class="btn btn-danger"><i class="nav-icon fa fa-trash"></i> Hapus</a>
                        </div>
                    </td>
                </tr>
                    <?php } ?>
        ...
        <!-- <div class="col-sm-8 col-lg-8 ">
            ...
            <table class="table table-striped table-hover table-bordered table-align-middle" id="data">
                ...
                <tr align="center">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="Deletedata_saldoPesantren.php?id=<?=$data['id_validasi']?>" onclick="return confirm('Apakah anda ingin menghapus data ini ?')" class="btn btn-danger"><i class="nav-icon fa fa-trash"></i> Hapus</a>
                        </div>
                    </td>
                </tr>
                    <?php } ?>
...
<?php
    ...
    if (isset($_POST['submit'])) {
        if (tambahValidasiPesantren($_POST) > 0){
            echo "
            <script>
    ...
    //     if (updateValidasiPesantren($_POST) > 0){
    //         echo "
    //         <script>
