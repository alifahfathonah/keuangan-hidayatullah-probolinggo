<?php
    ...
    $lokasi3 = "Validasi Pemasukan SD Integral";
    ...
    $namasumber1 = mysqli_query($koneksi,"SELECT * FROM `db_usaha` where id_usaha= 'DTU003' ");
    ...
    $hasil2=mysqli_query($koneksi,"SELECT * FROM db_data_output where id_usaha = 'DTU003'");
    ...
    $db_user= query("SELECT id_validasi, id_usaha, sub_total, anggaran FROM db_validasi WHERE id_validasi = 'VAL001'");
    $db_saldo = query("SELECT id_validasi, id_usaha, sub_total, anggaran FROM db_validasi WHERE id_validasi = 'VAN001'");


    $hasil3=mysqli_query($koneksi,"SELECT SUM(kd_jumlah) * 0.75 AS totalkeseluruhan FROM db_data_input  where id_usaha = 'DTU003'");
    while ($jumlah3=mysqli_fetch_array($hasil3)){
    $arrayhasil3[] = $jumlah3['totalkeseluruhan'];
    }
    $jumlahhasil3 = array_sum($arrayhasil3);

    $hasil4=mysqli_query($koneksi,"SELECT * FROM db_data_input  where id_usaha = 'DTU003'");
    while ($jumlah4=mysqli_fetch_array($hasil4)){
    $arrayhasil4[] = $jumlah4['kd_jumlah'];
    }
    $jumlahhasil4 = array_sum($arrayhasil4);

    $hasil5=mysqli_query($koneksi,"SELECT * FROM db_data_output  where id_usaha = 'DTU003'");
...
<br>
    <h2 align="center">Validasi Pemasukan SD Integral </h2>
        ...
        <div class="col-sm-8 col-lg-8 ">
            ...
            <table class="table table-striped table-hover table-bordered table-align-middle" id="data">
                ...
                <tr align="center">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="Deletedata_validasiSD.php?id=<?=$data['id_validasi']?>" onclick="return confirm('Apakah anda ingin menghapus data ini ?')" class="btn btn-danger"><i class="nav-icon fa fa-trash"></i> Hapus</a>
                        </div>
                    </td>
                </tr>
                    <?php } ?>
        ...
        <!-- <div class="col-sm-8 col-lg-8 ">
            ...
            <table class="table table-striped table-hover table-bordered table-align-middle" id="data">
                ...
                <tr align="center">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="Deletedata_saldoSD.php?id=<?=$data['id_validasi']?>" onclick="return confirm('Apakah anda ingin menghapus data ini ?')" class="btn btn-danger"><i class="nav-icon fa fa-trash"></i> Hapus</a>
                        </div>
                    </td>
                </tr>
                    <?php } ?>
...
<?php
    ...
    if (isset($_POST['submit'])) {
        if (tambahValidasi($_POST) > 0){
            echo "
            <script>
            alert('Validasi Data Berhasil');
            document.location.href = 'Validasi_SD.php';
            </script>
            ";
        } else {
            echo "
