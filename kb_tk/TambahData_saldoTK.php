<?php
    ...
    $hasil2=mysqli_query($koneksi,"SELECT * FROM db_validasi where id_usaha = 'DTU001'");
    while ($jumlah2=mysqli_fetch_array($hasil2)){
    $arrayhasil2[] = $jumlah2['sub_total'];
    }
    $jumlahhasil2 = array_sum($arrayhasil2);
    
    $hasil3=mysqli_query($koneksi,"SELECT SUM(anggaran) AS totalkeseluruhan FROM db_validasi where id_usaha = 'DTU001'");
    ...
    $hasil4=mysqli_query($koneksi,"SELECT anggaran * 0.80 AS totalkeseluruhan FROM db_validasi where id_usaha = 'DTU001'");
                ...
                <div class="card-body">
                    ...
                    <label for="">Kategori  </label>
                        <select name="id_kategori" class="form-control select-dropdown" required>
                            ...
                            $query = mysqli_query($koneksi, "SELECT * FROM db_kategori_pengeluaran WHERE id_usaha = 'DTU001' ORDER BY id_kategori DESC LIMIT 2");
...
<?php
    ...
    if (isset($_POST['submit'])) {
        if ($_POST['kd_jumlah'] <= $_POST['max']) {
            if (tambahdatapengeluaransaldoTK($_POST) > 0){
                echo "
                <script>
                alert('Pengeluaran Sudah Berhasil Di Tambah');
                document.location.href = 'EditData_pengeluaranTK.php';         
                </script>
                ";
            }   else {
                echo "
                ...
                document.location.href = 'TambahData_pengeluaranTK.php';         
                </script>
                ";
                echo("<br>");
