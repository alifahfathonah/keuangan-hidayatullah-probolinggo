<?php
    ...
    $kodetambahpengeluaranKB = kodetambahpengeluaranKB();
                ...
                <div class="card-body">
                    ...
                    <label for="">Kategori  </label>
                        <select name="id_kategori" class="form-control select-dropdown" required>
                            ...
                            $query = mysqli_query($koneksi, "SELECT * FROM db_kategori_pengeluaran WHERE id_usaha = 'DTU002' ORDER BY id_kategori DESC LIMIT 2");
...
<?php
    ...
    if (isset($_POST['submit'])) {
        if ($_POST['kd_jumlah'] <= $_POST['max']) {
            if (tambahdatapengeluaransaldoKB($_POST) > 0){
                echo "
                <script>
